...
//   // ✅ KEY FIX: always false on SSR → no mismatch. Reactive only after client mount.
...
// ✅ cn helper — produces clean single-line class strings, no whitespace drift
...
export default function Header() {
  ...
  // ✅ mount gate — isWhiteNav is always false on server, reactive on client
  ...
  // ✅ All classNames pre-built as flat strings — no template literal newlines
  const headerClass = cn(
    "fixed top-0 left-0 w-screen px-[10px] py-[16px] sm:py-[20px] z-[999999] transition-[background,transform] duration-300",
    showNav ? "translate-y-0" : "-translate-y-[110%]",
    isWhiteNav ? "bg-white backdrop-blur-[14px]" : "bg-transparent"
  );
  ...
  const asideClass = cn(
    "fixed top-0 right-0 h-screen w-[90vw] sm:w-[80vw] md:w-[60vw] lg:w-[40vw] xl:w-[30vw]",
    "bg-white/80 backdrop-blur-[12px] z-[1000000] transition-transform duration-[350ms]",
    menuOpen ? "translate-x-0" : "translate-x-full"
  );

  const barClass = cn(
    "w-[26px] sm:w-[28px] h-[3px] transition-all duration-300",
    isWhiteNav ? "bg-black" : "bg-white"
  );

  const dropdownClass = cn(
    open ? "flex" : "hidden",
    "group-hover:flex flex-col mt-2 rounded-md px-4 py-3 text-right w-fit"
  );
  ...
  return (
    <>
      ...
      <div onClick={closeMenu} className={backdropClass} />
      ...
      <aside className={asideClass}>
        ...
        <nav className="mt-[90px] sm:mt-[110px] pr-4 sm:pr-[20px] flex flex-col gap-3 sm:gap-[15px] text-right">
          <Link onClick={() => navigate("/")} href="/" prefetch className="text-[1.4rem] sm:text-[1.6rem] md:text-[2.2rem] lg:text-[2.6rem] font-bold hover:text-[#813DFF]">
            Home
          </Link>

          <div className="flex flex-col items-end group w-full">
            <div onClick={mobileOpen} className="flex items-center gap-2 text-[1.4rem] sm:text-[1.6rem] md:text-[2.2rem] lg:text-[2.6rem] font-bold cursor-pointer">
              Services
              <span className="text-[1.4rem] sm:text-[1.6rem] transition-transform duration-300 group-hover:rotate-180">
                <IoIosArrowDown />
              </span>
            ...
            <div className={dropdownClass}>
              <Link onClick={() => navigate("/services/digital-marketing")} href="/services/digital-marketing" prefetch className="text-[1.2rem] sm:text-[1.3rem] md:text-[1.4rem] lg:text-[1.5rem] font-medium hover:text-[#813DFF]">
                Digital Marketing
              </Link>
              <Link onClick={() => navigate("/services/ad-operations")} href="/services/ad-operations" prefetch className="mt-2 text-[1.2rem] sm:text-[1.3rem] md:text-[1.4rem] lg:text-[1.5rem] font-medium hover:text-[#813DFF]">
                Advertising Operations
              </Link>
            </div>
          </div>
