        ...
        {[0, 1].map((copy) => (
          <div key={copy} className="flex gap-17.5 pr-17.5">
            {items.map((p, i) => (
              ...
              >
                <div className="relative w-30 h-15">
                  {/* ✅ PERF FIX: Added loading="lazy" — these logos are below the fold
                      and should never block initial page load on mobile.
                      Added sizes="120px" — tells browser exact display size so it
                      downloads the correctly-sized image instead of a full-res version.
                      Zero visual change. */}
                  <Image
                    src={p.img}
                    alt={p.name}
