...
//       text: "Adaptive workflows and 24/7 availability ensure\nuninterrupted operations and long-term marketing momentum.",
//     },
//   ];
...
//           {points.map((p) => (
//             <div key={p.num} className="flex items-start gap-3 sm:gap-[10px] pb-[12px] sm:pb-0">
//               {/* NUMBER */}
//               <span
//                 className="
//                   font-[900]
//                   leading-[0.9]
//                   min-w-[72px] sm:min-w-[96px] lg:min-w-[110px]
//                   text-[4.5rem] sm:text-[4.6rem] lg:text-[5rem]
//                   ml-[-30px] sm:ml-0
//                   mr-[10px]       /* MOBILE ONLY */
...
export default function Features() {
  ...
  useEffect(() => {
    ...
    // ✅ FIXED: Only animate when visible — saves TBT + battery
    let isVisible = false;
    ...
    const setup = () => {
      ...
      const isMobile = window.innerWidth <= 768;
      // ✅ FIXED: Reduced node count (was 63, now 35 max) — less TBT
      nodes = Array.from({ length: isMobile ? 15 : 35 }, () => ({
        x: Math.random() * canvas.width,
        y: Math.random() * canvas.height,
        dx: (Math.random() - 0.5) * 1.1,
        dy: (Math.random() - 0.5) * 1.1,
    ...
    const animate = () => {
      ...
      // ✅ FIXED: Skip frames when off-screen
      if (!isVisible) return;
    ...
    <section className="relative w-screen overflow-hidden bg-[#c8df2f]">
      ...
      <div className="pointer-events-none absolute right-0 top-1/2 -translate-y-1/2 z-[1]">
        <Image
          ...
          alt=""
          aria-hidden="true"
          // ✅ FIXED: aria-hidden + empty alt for decorative image (accessibility)
          // ✅ FIXED: loading lazy since it's decorative and below fold
          loading="lazy"
          className="
            opacity-100
            w-[280px]
