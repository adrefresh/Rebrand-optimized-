...
export default function Footer() {
  return (
<footer className="w-full bg-[linear-gradient(to_bottom,#565656_0%,#949494_100%)] text-white">
      {/* ================= GRID ================= */}
      <div className="relative mx-auto max-w-350 px-20 py-15 grid grid-cols-4 gap-15 max-[1024px]:grid-cols-2 max-[640px]:grid-cols-1">
...
/>
          <div className="absolute -right-7.5 top-1/2 -translate-y-1/2 h-[70%] w-0.5 bg-white/40 hidden md:block" />
        </div>
...
/>
        ...
        <div className="grid grid-cols-2 gap-4 sm:gap-6 md:gap-8 text-sm sm:text-base leading-relaxed">
          {["India", "Australia", "USA", "Canada"].map((country) => (
            <div key={country} className="flex items-center gap-2 sm:gap-3">
              <MapPin className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8 lg:w-8.75 lg:h-8.75 shrink-0" />
              <span>{country}</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-6 max-[640px]:items-center mt-2">
          ...
          <div className="flex items-center gap-6">
            ...
            >
              ...
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-4.5 h-4.5 fill-current">
      ...
      {/* ================= DIVIDER ================= */}
      <div className="mx-auto h-0.5 w-[90%] bg-white/30" />
